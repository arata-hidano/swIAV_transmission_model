...
#-----------------------------------------------------------------------
# UPDATE: 26 May 2024
#       Modifying for boar service
#       Start with 2 boar, 9 sows, then farrow-to-finish production
#       Previously started simulation with random infection status
#       Now start with susceptible status then introduce infection with boar movement
#       Add boar movement: up to 20 times a year

# UPDATE: 2 June 2024
#       Adding immune duration
#       Duration at which all animals are immune?
#       In reality, after infection has disappeared, animals get immuned
#       How do we define immune farm? 
#       If the virus is sustained on farm, then some animals get infected multiple times, so we can assume that all animals are immune for immune period since the last day of the persistence
#       But persistence duration actually accounts for complex immune status where not all animals are susceptible and infected
#       So for immune period, we can calculate the time duration between the day virus disappears and at least one animal gets susceptible
#       This also relates to the introduction of new animals (e.g. birth) but these animals should be immune through MDA if dams are infected
...
Scenario_name = paste0("Type",farm_type, "_Ssize",toString(temp_init_sow),"_Bsize",toString(temp_init_boar),
                       "_initp",toString(prev_boar),"_R0",toString(r0_vector),
                       "_syn",synchro)
...
prev_farm = 0.1 # Need to stratify depending on farm types
    # Assuming that boar gets infected then 6 days infectious then 180 days immune, so about max 12 days in a year boars can be infectious
    # So in a given moment the boar can be infectious with 3% probability
...
length_simulation_day = 5*12*30 # unit is day

#============================================================
# It's ok to seed the infection at random time because some farms in reality are indeed just new farms that started
# So picking up a random seed point will mimic the reality where both new and old farms get infection
#============================================================


#==========================================================
# Below is the old demographic info based on literature
            # Set up demographic
...
# Here is the new parametrisation based on our field data
            #  # smallholder with sow = 3: 
            # sow1 (id 1) age 2. Farrow on Day 10 (MAY)
            # sow2 (id 2) age 3. Farrow on Day 40 (JUNE)
            # sow3 (id 3) age 4. Farrow on Day 70 (JULY)
            # each farrow, 12 piglets and some die before weaning. Weaning age 28 days
            # Mating happens throughout the year
            # sell as matured pig when reached age. Sows culled at age 5 or after they weaned.
            # Max 9 sows held and 3 sows farrowed at similar time
            # Boar lending 20 times/year/head
...
#========Do we need these parameters? They put weaners together and start raising them as growers
# Actually yes because this is period for gilts to be a reserve for the next sow
# Time from weaning to become grower (needs this info)
wean_to_grow = 28
# Time from weaning to become grower for Large company
wean_to_grow_L = 7

# Time from entering growing until finishing; for now 4 months (5 months age)
grow_to_finish = 84 # 140-56
...
# conception to farrow takes 4 months, so assuming the first service is 7mo it's 11mo age
grow_to_first_farrow = 270
...
gestation_period_min = 111
gestation_period_max = 120
...
# What affects persistence?
      # 1. Farrowing interval; type 1 focused farrow; type 2 random farrow
      # 2. Introduction of new animals during the outbreak
      # 3. Becoming susceptible during the outbreak (MDA dissapears mean 70 days); probably need to consider stochasticity in gestation
...
if(include_backyard==1)
{
  ...
  EMPTY_data_table = data.table(id = seq(1,N_INITIAL_BACKYARD), 
                                farm_id = integer(N_INITIAL_BACKYARD), 
                                ...
                                removal_date = integer(N_INITIAL_BACKYARD),
                                #immunity_date = integer(),  # REMOVED AND REPLACED BY PUTTING THIS INFO IN next_s_date
                                infection_history = integer(N_INITIAL_BACKYARD),
                                farrow_times = integer(N_INITIAL_BACKYARD),
                                alive = integer(N_INITIAL_BACKYARD)
  ...
  # Do these farm introduce external gilts?
  ...
  FARM_data_frame = data.table(farm_id = seq_along(1:n_backyard),
                               introduce_replace_gilt = introduce_replace_gilt,
                               N_PIGLET = integer(n_backyard),
                               N_WEANED = integer(n_backyard),
                               N_FATTENING = integer(n_backyard),
                               N_GILT = integer(n_backyard),
                               N_SOW = integer(n_backyard),
                               N_BOAR = integer(n_backyard),
                               N_TOTAL = integer(n_backyard),
                               N_SUSCEPTIBLE = integer(n_backyard),
  ...
  for(i in 1:n_backyard)
  {
    ...
    {
      temp_exposed = 1
      temp_susceptible = temp_total - 1
      temp_exposed_id = sample((temp_init_sow+1):temp_total,1)
      temp_status = rep(s_S,temp_total)
      temp_status[temp_exposed_id] = s_E
      temp_s_date = rep(0,temp_total)
    ...
    } else
    {
      ...
      temp_status = rep(s_S,temp_total)
      temp_s_date = rep(0,temp_total)
      temp_susceptible = temp_total
    }
    
    # ENTER INITIAL FARM CONDITION
    ...
    FARM_data_frame = FARM_data_frame[i,
                                      ':='(N_PIGLET=0,
                                           ...
                                           N_GILT = 0,
                                           N_SOW = temp_init_sow,
                                           N_BOAR =temp_init_boar, # start with 2 boars
                                           N_INFECTED = 0,
                                           N_IMMUNE = 0,
    ...
    init_sow_id= seq_along(1:temp_init_sow) + current_pig_id
    current_pig_id = current_pig_id + temp_init_sow
   
    #--------------------------------------------------------------------------------------------
    # INITIAL SOW CONDITION
    # Type 1: COnsecutive farrowing - farrow occurs in 2 months time (NOW)
    # Type 2: Random farrowing - farrow occurs randomly in 6 months
    first_farrow = sample(as.numeric(as.Date('2020-07-01') - start_date),length(init_sow_id),replace=T)
    ...
    init_sow_age = sample((init_sow_age_max-init_sow_age_min),length(init_sow_id),replace=T) + init_sow_age_min
    #SOW_data_frame$immunity_date = 0
    sow_mortality_date = (sample(cull_mo_min:cull_mo_max,length(init_sow_id),replace=T) - init_sow_age)*30
    ...
    ANIMAL_data_frame[init_sow_id, ":="(
                                                      ...
                                                      status = temp_status[1:length(init_sow_id)],
                                                      farrow_times = rep(0,length(init_sow_id)),
                                                      next_s_date = temp_s_date[1:length(init_sow_id)],
                                                      next_d_date = 0,  # when next_d_date >0 means it's milking (sows) or being milked (piglet)
                                                      alive = 1
    )]   
    
    #---------------------------------------------------------------------------------------------
    # INITIAL BOAR CONDITION IF PRESENT
    if(temp_init_boar>0)
    {
      init_boar_id = seq_along(1:temp_init_boar) + current_pig_id
      current_pig_id = current_pig_id + temp_init_boar
      
      # Define random boar infection date
      # Draw random date from length_simulation_day with a length of n_boar_lend_year
      # Or set a mean interval (which is 360/n_boar_lend_year), then adjust with standard deviation
      # For now uniform distribution with mean t = 360/n_boar_lend_year*2
      boar_movement = sample(360/n_boar_lend_year*2,length(init_boar_id),replace=F)
      
      # ENTER BOAR INFO
      # For now ignore culling boar @@@EXPAND need to consider boar replacement
      ANIMAL_data_frame[init_boar_id, ":="(
        farm_id = rep(i,length(init_boar_id)), 
        demographic = rep(d_boar,length(init_boar_id)),
        sex = rep(male,length(init_boar_id)), 
        status = temp_status[(temp_init_sow+1):temp_total],
        farrow_times = rep(0,length(init_boar_id)),
        next_s_date = temp_s_date[(temp_init_sow+1):temp_total],
        next_d_date = boar_movement,
        alive = 1
      )] 
    }
...
if(include_backyard==1)
{
  ...
  #======================================================================================================
    ...
    TEMP_FARROW_dat = TEMP_ANIMAL_data_frame1[farrow_date==0 & id <= current_pig_id,c("id","farm_id","status")]
    ...
    if(temp_nrow>0)
    {
      ...
      current_piglet = 1
      # NOW VISIT EACH FARROWING PIGS
      for(i in 1:temp_nrow)
      {
        temp_status = TEMP_FARROW_dat[i,status]
        nfarm =  TEMP_FARROW_dat[i,farm_id]
        n_piglet = n_piglet_vec[i]
        day_piglet_mortality[current_piglet:(current_piglet+n_piglet-1)] = c(sample(1:wean_day_min,n_piglet_mortality_vec[i],replace=T),rep(-1,n_piglet-n_piglet_mortality_vec[i]))
    ...
    #===SOW PREGNANT EVENT=====================================
          # NOW ADD TRANSMISSION FROM BOAR USE
    temp_pregnant_data = TEMP_ANIMAL_data_frame1[((farrow_date==-1*return_to_heat)|((-1*farrow_date)-return_to_heat)%%30==0) & 
                                                   (demographic==d_gilt|demographic==d_sow),id] 
    if(length(temp_pregnant_data)>0)
    { 
      # What's the transmission probability? Infection probability of boar times transmission rate (the latter may be 1)
      # Get only sows that are susceptible
      temp_pregnant_sus_id = ANIMAL_data_frame[(id %in% temp_pregnant_data) & status == s_S,id]
      temp_infection = rbinom(length(temp_pregnant_sus_id),1,prev_boar) # assign new infection for susceptible sows
      id_to_update_inf = temp_pregnant_sus_id[temp_infection==1] # id of sows to become infected
      # farm_id_to_update_inf = unique(ANIMAL_data_frame[id %in% id_to_update_inf,farm_id]) # farm id of sows to be infected
        
        # For each sow to be infected update the status
      if(length(id_to_update_inf)>0)
      {
        for(sus_sow in id_to_update_inf)
        {
          ANIMAL_data_frame[id==sus_sow,":="
                                        (status=s_E,
                                          next_s_date = day_latent
                                        )
                                          ]
          temp_farm_id_inf = ANIMAL_data_frame[id==sus_sow,farm_id] # extract farm id then update farm infection status
          FARM_data_frame[farm_id == temp_farm_id_inf,
      ...
      # Now update pregnant status
      temp_mating = rbinom(length(temp_pregnant_data),1,prob_mating) # MATING SUCCESS OR NOT
      id_to_update = temp_pregnant_data[temp_mating==1]
      if(length(id_to_update)>0)
      {
        temp_gestation_period = sample(gestation_period_min:gestation_period_max,length(id_to_update),replace=T)
        ANIMAL_data_frame[id %in% id_to_update,farrow_date := temp_gestation_period]
      }
      ...
      for(animal in 1:NROW(temp_demographic_data))
      {
        ...
        if(temp_demographic_data[animal,next_d_date]==0)
        {
          ...
          else if(current_demographic==d_weaned)
          {
            ...
            tem_next_d_date = 0 # as no more demographic change occurs
            # UPDATE FARM TABLE
            FARM_data_frame[temp_farm_id,
                            ":="(N_WEANED = N_WEANED -1,
          ...
          else if(current_demographic==d_gilt) # GILT BECOMING SOW
          {
            ...
            tem_next_d_date = 0 # @@@Expand check if needed to put next_d_date for gilts and need to include any events for sows
            # UPDATE FARM TABLE
          ...
          else if (current_demographic==d_boar) # BOAR BRINGING INFECTION
          {
            # Get the id of boar and check infection status
            # If immune nothing happens, if susceptible then become exposed
            # UPDATE: If this is BSP then check boar status, if boar not susceptible infection doesn't happen
            # If non_BSP then if any animals are susceptible ifnection happens
            ...
            if(BSP_yes == 1)
              ...
              ind_infection = rbinom(1,1,prev_boar) # Determine if boars get infected 
                      ...
                      } # if not susceptible then nothing happens
                      
                      } 
      
          
            # Then determine the next movement date
            temp_boar_movement = sample(360/n_boar_lend_year*2,1,replace=F) + 1
            # Determine the next movement date
            ANIMAL_data_frame[id==temp_id,next_d_date:= temp_boar_movement
                              ]
            }
        ...
        if( temp_demographic_data[animal,next_s_date]==0)
        {
          ...
          {
            next_status = s_I
            temp_next_s_date = day_infectious
